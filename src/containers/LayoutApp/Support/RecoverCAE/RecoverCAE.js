...
import { actionDoRecoverCAE } from "./../../../../actions/supportActions";
...
const RecoverCAE = (props) => {
  ...
  const { handleChange, handleSubmit, values, errors } = useForm({
    ...
    formBody: { idItem: "", selectItem: "", pointSale: "", voucherNumber: "" },
  });
   //useForm
  ...
  React.useEffect(() => {
    switch (props.recoverCAE.recoverCAEStatus) {
      case "FETCHING":
  ...
  return (
    ...
    {notification.active && (
      ...
      {props.recoverCAE.recoverCAEErrorMessage && (
        <Grid container spacing={4}>
          <Grid item xs={12}>
      ...
      <Modal active={modalstate.active}>
          <Modal.Body>
            ¿Está seguro que desea actualizar los del CUIT seleccionado?
...
const mapStateToProps = (state) => ({
  recoverCAE: state.supportReducer,
});

function mapDispatchToProps(dispatch) {
  return bindActionCreators(
    {
      doRecoverCAE: actionDoRecoverCAE,
    },
    dispatch
  );
}

export default connect(mapStateToProps, mapDispatchToProps)(RecoverCAE);
